...
When the user asks to "open" one or more files, do not read them into this session or print them. Instead open them in a Neovim editor pane so the user can view and edit them.

Create a new vertical pane next to the caller, preserving the caller's working directory, and read the new pane ID from `.result.pane.pane_id`:

```bash
herdr pane split --current --direction right --cwd "$PWD"
```

A vertical split places the new pane to the `right`. Keep the user's focus in the editor unless they asked otherwise; omit `--no-focus` here so they land in the file.

Then send an `nvim` command that opens the requested files. Pass every requested file as an argument so Neovim loads them together (as buffers, with the first shown and the rest available via `:bnext` or listed with `:ls`):

```bash
herdr pane run <returned-pane-id> "nvim path/to/first.ts path/to/second.ts"
```

Quote paths that contain spaces. For a single file, pass just that one path. Use paths relative to the preserved working directory, or absolute paths when clearer.
