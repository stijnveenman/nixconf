...
export default function (pi: ExtensionAPI) {
  const runHandoff = async (ctx: ExtensionCommandContext | ExtensionContext) => {
    const commandContext = ctx as ExtensionCommandContext;
    const state = await showHandoffMenu(commandContext);
    if (!state?.agent || !state.branch) return;

    const agent = state.agent;
    const branch = state.branch;
    pi.appendEntry("pi-handoff-task", { task: state.task });
    const result = await runTask(commandContext, {
      ...
      task: async () =>
        addWorkmuxAgent(
          ...
          state.taskContext === "fork"
            ? commandContext.sessionManager.getSessionId()
            : undefined,
        ),
  ...
  pi.registerCommand("handoff", {
    ...
    handler: async (_args, ctx) => runHandoff(ctx),
  });
